...
using Banking.Account.Command.Application.Contratcs.Persistence;
        ...
        public async Task SaveEvents(string aggregateId, IEnumerable<BaseEvent> events, int expectedVersion)
        {
            ...
            foreach(var evt in events)
            {
                ...
                var eventModel = new EventModel
                {
                    ...
                    AgregateType = nameof(AccountAggregate),
                    Version = version,
                ...
                await _eventStoreRepository.InsertDocument(eventModel); // para guardar en mongo
                _eventProducer.Produce(evt.GetType().Name, evt); // Para enviarlo a kafka
