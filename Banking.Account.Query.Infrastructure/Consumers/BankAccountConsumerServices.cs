...
{
    public class BankAccountConsumerServices : IHostedService
    {
        ...
        public BankAccountConsumerServices(IServiceScopeFactory factory)
        {
                    ...
                    try
                    {
                        while (true)
                        {
                            ...
                            if (consumer.Topic == typeof(AccountOpenedEvent).Name)
                            {
                                ...
                                {
                                    Identifier = accountOpenedEvent.AccountHolder,
                            ...
                            {
                                var accountOpenedEvent = JsonConvert.DeserializeObject<AccountClosedEvent>(consumer.Message.Value);

                                _bankAccountRepository.DeleteByIdentifier(accountOpenedEvent!.Id).Wait();
                            }
                            ...
                            {
                                var accountDepositEvent = JsonConvert.DeserializeObject<FundsWithdrawnEvent>(consumer.Message.Value);
